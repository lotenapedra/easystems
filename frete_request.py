...
if st.button("Salvar"):
    try:
        ...
        cursor.execute('''INSERT INTO frete (
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                        (data_solicitacao_value, empresa_origem_value, estado_origem_value, cidade_origem_value, estado_destino_value, cidade_destino_value, tipo_veiculo_value, data_coleta_value, data_entDesculpe, mas parece que houve um erro ao executar o código fornecido. O trecho de código fornecido tem um comentário `streamlit run gestao_fretes.py --server.enableCORS=false --browser.serverAddress=0.0.0.0 --browser.gatherUsageStats=false --ssl-version=TLSv1` após a importação de pacotes, que não é válido e está causando o `SyntaxError`.
